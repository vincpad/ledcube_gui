...
public void fftVisualizer() {
  if(fileselected == true) {
    ...
    myCube.clear();
    float[] coef = new float[]{0.3f, 0.6f, 1, 2, 4, 4.5f, 6, 9};
    for(int i = 0; i < 8; i++) {
        float valTmp = fft.getAvg(i)*coef[i];
  
        int val = PApplet.parseInt(valTmp);
        if(val>8) {
          val = 8;
        }
...
// Dev functions, used to measure the time taken for some actions
...
public void e(int n) {
  ...
  println("Time taken by action "+ n + " : " + (endTime - startTime) + " ms" );
}
//SNAKE 3D
//
// Part of ledcube_gui project : https://github.com/cybervinc/ledcube_gui
